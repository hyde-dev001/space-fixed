...
class CheckLowStockJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($inventoryItems as $item) {
            ...
            if ($currentQuantity <= 0) {
                ...
                $this->createAutomaticStockRequestIfEnabled($item, 'high');
                $outOfStockCount++;
                continue;
            }
            ...
            if ($currentQuantity > 0 && $currentQuantity <= $reorderLevel) {
                ...
                $this->createAutomaticStockRequestIfEnabled($item, 'medium');
                $lowStockCount++;
                continue;
            }
    ...
    protected function createAutomaticStockRequestIfEnabled(InventoryItem $item, string $priority): void
    {
        if (! $item->auto_stock_request_enabled) {
            return;
        }

        app(StockRequestApprovalService::class)->createAutomaticStockRequest($item, $priority);
    }
