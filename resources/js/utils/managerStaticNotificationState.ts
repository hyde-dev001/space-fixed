...
import { MANAGER_STATIC_NOTIFICATIONS } from './managerStaticNotifications';

const STORAGE_KEY = 'manager_static_notification_read_ids';
const ARCHIVED_STORAGE_KEY = 'manager_static_notification_archived_ids';
const EVENT_NAME = 'manager-static-notifications-updated';
...
export const getManagerStaticNotificationsWithReadState = (archivedOnly = false): Notification[] => {
  const readIds = new Set(getStoredIds(STORAGE_KEY));
  const archivedIds = new Set(getStoredIds(ARCHIVED_STORAGE_KEY));

  return MANAGER_STATIC_NOTIFICATIONS
    .map((notification) => ({
      ...notification,
...
export const markManagerStaticNotificationAsRead = (id: number) => {
  const ids = getStoredIds(STORAGE_KEY);
  if (ids.includes(id)) return;
  saveStoredIds(STORAGE_KEY, [...ids, id]);
};

export const markAllManagerStaticNotificationsAsRead = () => {
  saveStoredIds(STORAGE_KEY, MANAGER_STATIC_NOTIFICATIONS.map((notification) => notification.id));
};

export const archiveManagerStaticNotification = (id: number) => {
  const ids = getStoredIds(ARCHIVED_STORAGE_KEY);
  if (ids.includes(id)) return;
  saveStoredIds(ARCHIVED_STORAGE_KEY, [...ids, id]);
};

export const unarchiveManagerStaticNotification = (id: number) => {
  const ids = getStoredIds(ARCHIVED_STORAGE_KEY);
  if (!ids.includes(id)) return;
  saveStoredIds(ARCHIVED_STORAGE_KEY, ids.filter((archivedId) => archivedId !== id));
};

export const MANAGER_STATIC_NOTIFICATIONS_EVENT = EVENT_NAME;
