...
import { REPAIRER_STATIC_NOTIFICATIONS } from './repairerStaticNotifications';

const STORAGE_KEY = 'repairer_static_notification_read_ids';
const ARCHIVED_STORAGE_KEY = 'repairer_static_notification_archived_ids';
const EVENT_NAME = 'repairer-static-notifications-updated';
...
export const getRepairerStaticNotificationsWithReadState = (archivedOnly = false): Notification[] => {
  const readIds = new Set(getStoredIds(STORAGE_KEY));
  const archivedIds = new Set(getStoredIds(ARCHIVED_STORAGE_KEY));

  return REPAIRER_STATIC_NOTIFICATIONS
    .map((notification) => ({
      ...notification,
...
export const markRepairerStaticNotificationAsRead = (id: number) => {
  const ids = getStoredIds(STORAGE_KEY);
  if (ids.includes(id)) return;
  saveStoredIds(STORAGE_KEY, [...ids, id]);
};

export const markAllRepairerStaticNotificationsAsRead = () => {
  saveStoredIds(STORAGE_KEY, REPAIRER_STATIC_NOTIFICATIONS.map((notification) => notification.id));
};

export const archiveRepairerStaticNotification = (id: number) => {
  const ids = getStoredIds(ARCHIVED_STORAGE_KEY);
  if (ids.includes(id)) return;
  saveStoredIds(ARCHIVED_STORAGE_KEY, [...ids, id]);
};

export const unarchiveRepairerStaticNotification = (id: number) => {
  const ids = getStoredIds(ARCHIVED_STORAGE_KEY);
  if (!ids.includes(id)) return;
  saveStoredIds(ARCHIVED_STORAGE_KEY, ids.filter((archivedId) => archivedId !== id));
};

export const REPAIRER_STATIC_NOTIFICATIONS_EVENT = EVENT_NAME;
