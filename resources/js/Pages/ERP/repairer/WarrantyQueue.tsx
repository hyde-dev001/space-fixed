...
export default function WarrantyQueue() {
  ...
  const fetchWarrantyClaimQueue = async (status: WarrantyStatusFilter) => {
    try {
      ...
      const response = await axios.get("/api/repairer/warranty-claims", {
  ...
  const fetchWarrantyKpi = async () => {
    try {
      const response = await axios.get("/api/repairer/warranty-claims/kpi", {
        params: { days: 30 },
      });
  ...
  const handleWarrantyClaimDecision = async (
    ...
    try {
      await axios.post(`/api/repairer/warranty-claims/${claim.id}/${action}`, payload);
  ...
  return (
    <AppLayoutERP hideHeader={isClaimDetailsOpen}>
