...
export default function StockRequest() {
	...
	const getRequestSourceMeta = (source?: string, serverLabel?: string, isAutoGenerated = false) => {
		if (isAutoGenerated) {
			return {
				label: serverLabel || 'Automatic Stock Request',
				className: 'border border-blue-200 bg-blue-100 text-blue-700 dark:border-blue-800 dark:bg-blue-900/30 dark:text-blue-300',
			};
		}
	...
	const getRequestSourceMetaForRequest = (request: StockRequestApproval) => {
		return getRequestSourceMeta(request.request_source, request.source_label, request.is_auto_generated);
	};

	const getRequesterRoleLabel = (request: StockRequestApproval) => {
		if (request.is_auto_generated) return 'System';
								...
								{loading ? (
									...
									paginatedItems.map((request) => (
										<tr key={request.id} className="odd:bg-white even:bg-gray-50/40 hover:bg-blue-50/40 dark:odd:bg-transparent dark:even:bg-gray-800/20 dark:hover:bg-gray-800/40 transition-colors">
											...
											<td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
												<div className="flex flex-col gap-1">
													<span className={`inline-flex w-fit min-w-24 items-center justify-center whitespace-nowrap rounded-full px-2.5 py-1 text-xs font-semibold leading-none ${getRequestSourceMeta(request.request_source, request.source_label, request.is_auto_generated).className}`}>
														{getRequestSourceMeta(request.request_source, request.source_label, request.is_auto_generated).label}
													</span>
						...
						<div className="overflow-y-auto max-h-[calc(92vh-154px)] px-5 py-5 sm:px-6 sm:py-6">
							<div className="space-y-5">
								...
								<div className="rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-800/40">
									...
									<p className="mt-1 text-base font-semibold text-gray-900 dark:text-white">{getRequestSourceMetaForRequest(viewingRequest).label}</p>
