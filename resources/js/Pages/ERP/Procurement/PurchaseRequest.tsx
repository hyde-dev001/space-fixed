...
type PurchaseRequestStatus = "Draft" | "Pending Finance" | "Approved" | "Rejected";
...
const supplierOptions: SupplierOption[] = [
	{ id: 1, name: "Metro Footwear Trading", contact: "0917-456-1188" },
	{ id: 2, name: "Prime Shoe Goods", contact: "0922-315-7721" },
	{ id: 3, name: "Urban Streetwear Partners", contact: "0956-123-6670" },
	{ id: 4, name: "CleanKicks Supply Co.", contact: "0939-884-2310" },
];
...
const purchaseRequestRows: PurchaseRequestItem[] = [
	{
		...
		justification: "Out of stock with high branch demand for weekend sales.",
	},
	{
		id: 2,
		prNo: "PR-2026-002",
		...
		status: "Approved",
		justification: "Needed for approved stock request from inventory.",
	},
	{
		id: 3,
		prNo: "PR-2026-003",
		...
		status: "Draft",
		justification: "For scheduled monthly replenishment of care products.",
	},
];
...
const statusBadgeClass: Record<PurchaseRequestStatus, string> = {
	...
	"Pending Finance": "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-300",
...
export default function PurchaseRequest() {
	const [purchaseRequests, setPurchaseRequests] = useState<PurchaseRequestItem[]>(purchaseRequestRows);
	...
	const pendingFinanceCount = purchaseRequests.filter((request) => request.status === "Pending Finance").length;
	...
	const handleCreatePR = async () => {
		if (!formData.productName.trim() || !formData.supplierName.trim() || !formData.quantity.trim() || !formData.unitCost.trim() || !formData.justification.trim()) {
			await Swal.fire({
				icon: "warning",
				title: "Missing fields",
				text: "Please complete all required fields before sending to Finance.",
		...
		const nextId = Math.max(...purchaseRequests.map((item) => item.id), 0) + 1;
		const prNo = `PR-2026-${String(nextId).padStart(3, "0")}`;

		const newPurchaseRequest: PurchaseRequestItem = {
			id: nextId,
			prNo,
		...
		setPurchaseRequests((prev) => [newPurchaseRequest, ...prev]);
		setCurrentPage(1);
		closeCreateModal();
		...
		await Swal.fire({
			...
			text: `${prNo} has been submitted for finance approval.`,
			confirmButtonColor: "#2563eb",
			timer: 1500,
		...
		<AppLayoutERP hideHeader={isAnyModalOpen}>
			...
			<div className="p-6 space-y-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					<MetricCard title="Total PR" value={totalPrCount} description="Purchase requests created" icon={ClipboardIcon} color="info" />
					<MetricCard title="Pending Finance" value={pendingFinanceCount} description="Requests waiting approval" icon={ClockIcon} color="warning" />
					<MetricCard title="Approved" value={approvedCount} description="Finance-approved requests" icon={CheckCircleIcon} color="success" />
				</div>
								...
								{paginatedItems.length > 0 ? (
									paginatedItems.map((request) => (
										<tr key={request.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
											...
											<td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300 whitespace-nowrap">{currency.format(request.unitCost)}</td>
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center p-4">
					...
					<div className="relative w-full max-w-2xl rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-xl">
						...
						<div className="p-6 space-y-4">
							...
							<div>
								...
								<select
									...
									onChange={(event) => setFormData((prev) => ({ ...prev, supplierName: event.target.value }))}
									className="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:border-blue-500 dark:focus:border-blue-400"
								>
									<option value="">Select supplier</option>
									{supplierOptions.map((supplier) => (
										<option key={supplier.id} value={supplier.name}>{supplier.name} ({supplier.contact})</option>
									))}
								</select>
							</div>
						...
						<div className="flex gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
							...
							<button onClick={handleCreatePR} className="flex-1 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition-colors">Send to Finance</button>
						</div>
					</div>
				</div>
			)}
