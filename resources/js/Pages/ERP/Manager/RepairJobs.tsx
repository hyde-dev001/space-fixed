...
type DecisionMode = "reassign" | "final-reject";
...
const DecisionDialog = ({
    ...
    const title = isReassign ? `Reassign ${repair.request_id}` : `Reject ${repair.request_id}`;
    const actionLabel = isReassign ? "Confirm reassignment" : "Confirm final rejection";
            ...
            <form onSubmit={onSubmit} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
                <div className="flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">{isReassign ? "This is an exception handoff after rejection or repairer unavailability. It does not mean the Manager will perform the repair." : "This closes the repair request as rejected by Manager. It does not create a refund or other financial remedy."}</p>
                    </div>
    ...
    return (
        <AppLayoutERP>
            ...
            <main className="space-y-6 py-6 md:py-8" aria-labelledby="manager-repair-jobs-title">
                ...
                <ManagerFilterPanel title="Filter repair workload" titleId="repair-filters-title" description="Review pending includes repairer rejections and repairer-unavailability exceptions." metadata={<><p className="font-semibold text-gray-900 dark:text-white">{payload?.total ?? 0} repairs in view</p><p className="mt-1 text-gray-500 dark:text-gray-400">Last updated: {formatDateTime(repairs.data?.last_updated_at)}</p></>}><form onSubmit={(event) => event.preventDefault()} className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6"><div className="xl:col-span-2"><label htmlFor="repair-search" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Search</label><input id="repair-search" type="search" value={form.search} onChange={(event) => setForm((current) => ({ ...current, search: event.target.value }))} placeholder="Request ID or customer" className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></div><div><label htmlFor="repair-status" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Status</label><MonochromeSelect id="repair-status" value={form.status} onChange={(event) => setForm((current) => ({ ...current, status: event.target.value }))} className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white"><option value="">All statuses</option><option value="assigned_to_repairer">Assigned</option><option value="in_progress">In progress</option><option value="repairer_rejected">Repairer rejected</option><option value="reassignment_required">Reassignment required</option><option value="awaiting_assignment">Awaiting assignment</option><option value="rejected">Rejected</option></MonochromeSelect></div><div><label htmlFor="repair-assignment-state" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Assignment</label><MonochromeSelect id="repair-assignment-state" value={form.assignment_state} onChange={(event) => setForm((current) => ({ ...current, assignment_state: event.target.value }))} className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white"><option value="">All assignment states</option><option value="reassignment_required">Reassignment required</option><option value="awaiting_assignment">Awaiting assignment</option></MonochromeSelect></div><div><label htmlFor="repairer-id" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">Repairer ID</label><input id="repairer-id" inputMode="numeric" value={form.repairer_id} onChange={(event) => setForm((current) => ({ ...current, repairer_id: event.target.value }))} placeholder="Optional" className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></div><div><label htmlFor="repair-date-from" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">From</label><input id="repair-date-from" type="date" value={form.date_from} onChange={(event) => setForm((current) => ({ ...current, date_from: event.target.value }))} className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></div><div><label htmlFor="repair-date-to" className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-300">To</label><input id="repair-date-to" type="date" value={form.date_to} onChange={(event) => setForm((current) => ({ ...current, date_to: event.target.value }))} className="min-h-11 w-full rounded-lg border border-gray-300 bg-white px-3 text-sm text-gray-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-white" /></div><label className="flex min-h-11 items-center gap-2 self-end text-sm font-medium text-gray-700 dark:text-gray-300"><input type="checkbox" checked={form.review_pending} onChange={(event) => setForm((current) => ({ ...current, review_pending: event.target.checked }))} className="h-4 w-4 rounded border-gray-300 text-blue-700 focus:ring-blue-500" />Review pending</label><label className="flex min-h-11 items-center gap-2 self-end text-sm font-medium text-gray-700 dark:text-gray-300"><input type="checkbox" checked={form.overdue} onChange={(event) => setForm((current) => ({ ...current, overdue: event.target.checked }))} className="h-4 w-4 rounded border-gray-300 text-blue-700 focus:ring-blue-500" />Overdue only</label><ManagerFilterActions><button type="button" onClick={clearFilters} className="min-h-11 rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">Clear</button></ManagerFilterActions></form></ManagerFilterPanel>
