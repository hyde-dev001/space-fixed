...
const SuspendAccount: React.FC = () => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to load suspension requests',
  ...
  const confirmApprove = async () => {
    ...
    try {
      const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
      const response = await fetch(`/api/manager/suspension-requests/${selectedRequest.id}/review`, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
      ...
      Swal.fire({
        icon: 'success',
        title: 'Approved!',
        text: data.message || 'Suspension request approved. Forwarded to shop owner.',
  ...
  const confirmReject = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to reject suspension request');
      }
      ...
      Swal.fire({
        icon: 'success',
        title: 'Rejected',
        text: data.message || 'Suspension request rejected',
        confirmButtonColor: '#ef4444',
      });
  ...
  return (
    <AppLayoutERP>
      <Head title="Suspend Account Manager" />

      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-2">
            Suspend Account Manager
          </h1>
