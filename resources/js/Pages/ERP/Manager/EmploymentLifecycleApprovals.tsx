...
interface LifecycleRequest {
  ...
  rehire_functional_role?: string | null;
...
const statusLabel = (value: string): string => value.replace(/[_-]+/g, " ").replace(/\b\w/g, (character) => character.toUpperCase());
...
export default function EmploymentLifecycleApprovals() {
  ...
  const review = async (request: LifecycleRequest, action: "approve" | "reject", reason = "") => {
    ...
    try {
      ...
      setActionError(caught instanceof Error ? caught.message : "Unable to save this decision.");
    } finally {
      setProcessingId(null);
    }
  };

  const approve = (request: LifecycleRequest) => {
    if (window.confirm("Approve the Manager stage for " + (request.name || "this employee") + "? The request will move to Company Shop Owner review.")) {
  ...
  const reject = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (rejecting && note.trim().length >= 3) void review(rejecting, "reject", note.trim());
  };
  ...
  return (
    <AppLayoutERP>
          ...
          <section className="space-y-4" aria-label={label + " request queue"}>
            {requests.map((request) => (
              <article key={request.id} className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => setSelected(request)} className="min-h-11 rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">View notes</button><button type="button" onClick={() => approve(request)} disabled={processingId === request.id} className="min-h-11 rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white hover:bg-emerald-800 disabled:opacity-50">Approve stage</button><button type="button" onClick={() => { setRejecting(request); setNote(""); }} disabled={processingId === request.id} className="min-h-11 rounded-lg border border-red-200 px-4 py-2 text-sm font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50 dark:border-red-800 dark:text-red-300">Reject request</button></div>
              </article>
            ))}
          </section>
        )}
      ...
      {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/50 p-4"><div role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-wide text-blue-600">{label} request #{selected.id}</p><h2 className="mt-1 text-xl font-semibold text-gray-900 dark:text-white">{selected.name || "Unknown employee"}</h2></div><button type="button" onClick={() => setSelected(null)} className="min-h-11 rounded-lg border border-gray-300 px-3 text-sm font-semibold dark:border-gray-700 dark:text-gray-300">Close</button></div><div className="mt-5 space-y-4 text-sm text-gray-700 dark:text-gray-300"><div><h3 className="font-semibold text-gray-900 dark:text-white">HR reason</h3><p className="mt-1 whitespace-pre-wrap leading-6">{selected.reason}</p></div><div><h3 className="font-semibold text-gray-900 dark:text-white">HR evidence / details</h3><p className="mt-1 whitespace-pre-wrap leading-6">{selected.evidence || "No evidence supplied."}</p></div>{selected.manager_note && <div><h3 className="font-semibold text-gray-900 dark:text-white">Manager note</h3><p className="mt-1 whitespace-pre-wrap leading-6">{selected.manager_note}</p></div>}{selected.owner_note && <div><h3 className="font-semibold text-gray-900 dark:text-white">Owner note</h3><p className="mt-1 whitespace-pre-wrap leading-6">{selected.owner_note}</p></div>}</div></div></div>}
      {rejecting && <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-950/50 p-4"><div role="dialog" aria-modal="true" className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-900"><h2 className="text-lg font-semibold text-gray-900 dark:text-white">Reject {label.toLowerCase()} request</h2><p className="mt-2 text-sm text-gray-600 dark:text-gray-400">A rejection closes the request at the Manager stage and does not change the employee account.</p><form onSubmit={reject} className="mt-5 space-y-4"><textarea aria-label="Rejection reason" value={note} onChange={(event) => setNote(event.target.value)} required minLength={3} maxLength={1000} rows={5} className="w-full rounded-lg border border-gray-300 bg-white p-3 text-sm dark:border-gray-700 dark:bg-gray-950 dark:text-white" placeholder="Reason for rejection" /><div className="flex justify-end gap-2"><button type="button" onClick={() => setRejecting(null)} className="min-h-11 rounded-lg border border-gray-300 px-4 py-2 text-sm font-semibold dark:border-gray-700 dark:text-gray-300">Cancel</button><button type="submit" disabled={note.trim().length < 3 || processingId === rejecting.id} className="min-h-11 rounded-lg bg-red-700 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Confirm rejection</button></div></form></div></div>}
    </AppLayoutERP>
  );
}
