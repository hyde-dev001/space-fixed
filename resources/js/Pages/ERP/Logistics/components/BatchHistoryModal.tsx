...
export default function BatchHistoryModal({ batches, isOpen, onClose, onOpen, onDetails, onRestore }: Props) {
  ...
  return <Modal isOpen={isOpen} onClose={onClose} size="7xl" showCloseButton={false}>
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="batch-history-title" onKeyDown={handleKeys} className="max-h-[90vh] overflow-y-auto p-5 sm:p-7">
      <div className="flex items-start justify-between gap-4">
        ...
        <button ref={closeRef} type="button" onClick={onClose} aria-label="Close batch history" className="min-h-10 rounded-lg border px-3 text-sm font-semibold text-gray-700 hover:bg-gray-50">Close</button>
      </div>
