...
export default function Shipments({ children }: React.PropsWithChildren) {
  ...
  const updateFilter = (key: keyof ShipmentFilters, value: string) => {
    ...
    const ownerShipmentsUrl = erpUrl(erpCapabilities, 'GET:erp.logistics.shipments');
    const shipmentsUrl = ownerShipmentsUrl
    ...
    const deliveriesUrl = erpUrl(erpCapabilities, 'GET:erp.logistics.deliveries')
      ?? (ownerMode && riderMode
        ? ownerShipmentsUrl
        : (!ownerMode && riderMode && canViewShipments ? '/erp/logistics/deliveries' : null));
    const targetUrl = riderMode ? deliveriesUrl : shipmentsUrl;
    const targetParams = ownerMode && riderMode
      ? { ...next, rider_mode: 1 }
      : next;

    if (!targetUrl) return;

    router.get(targetUrl, targetParams, {
      preserveScroll: true,
      preserveState: true,
    });
  };
  ...
  const resolveCustomerDispute = async (disputeId: number, reason: string) => {
    const resolution = await Swal.fire({
      title: 'Resolve customer dispute',
      input: 'select',
      inputOptions: {
        ...(reason === 'item_not_received' ? { customer_confirmed: 'Customer confirmed' } : {}),
                ...
                >
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          {!ownerMode && !riderMode && canResolveDisputes && ['open', 'investigating'].includes(dispute.status) && (
                            <div className="flex flex-wrap gap-2">
                              ...
                              {dispute.status === 'investigating' && <button type="button" onClick={() => void resolveCustomerDispute(dispute.id, dispute.reason)} className="rounded-lg bg-rose-700 px-3 py-2 text-xs font-semibold text-white hover:bg-rose-800">Resolve</button>}
                            </div>
                          )}
                        </div>
                  ...
                  <div className="mt-4 space-y-4 sm:mt-5">
                    {(shipment.legs ?? []).map((leg) => {
                            ...
                            const showOutcomeChoice = canSubmitProof && canReportIssueForLeg;
                            ...
                            return (
                              <div key={leg.id} className="grid min-w-0 gap-5 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 dark:shadow-none sm:p-5 lg:grid-cols-[minmax(0,1fr)_minmax(18rem,22rem)] lg:rounded-lg lg:p-4 lg:shadow-none">
                                ...
                                <div className="flex min-w-0 flex-col gap-3 rounded-2xl border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-900/40 lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0">
                                  ...
                                  {canReportIssueForLeg && (!showOutcomeChoice || deliveryOutcome === 'issue') && (
