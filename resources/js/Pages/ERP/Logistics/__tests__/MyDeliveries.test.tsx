...
describe('MyDeliveries rider interactions', () => {
  ...
  it('uses reason-specific issue evidence and allows unsafe reporting without a photo', async () => {
    ...
    expect(screen.getByRole('link', { name: 'Text' })).toHaveClass('border-slate-300', 'text-slate-950');
    expect(screen.getByRole('link', { name: 'Text' })).toHaveAttribute('href', 'sms:090015');
  ...
  it('keeps the canonical delivery actionable and explains that conflicting work is blocked', () => {
    ...
    expect(screen.getByRole('link', { name: 'Text' })).toHaveAttribute('href', 'sms:09007');
    expect(screen.queryByRole('link', { name: 'Directions' })).not.toBeInTheDocument();
  });
