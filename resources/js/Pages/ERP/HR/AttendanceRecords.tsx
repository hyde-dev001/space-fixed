...
const parseTimeTo24Hour = (timeValue: string | null | undefined): string | null => {
...
const formatTimeTo12Hour = (timeValue: string | null | undefined): string => {
  const normalized24 = parseTimeTo24Hour(timeValue);
  if (!normalized24) return "-";

  const [hourString, minute] = normalized24.split(":");
...
const transformAttendanceFromApi = (apiRecord: any) => {
  ...
  return {
    ...
    checkInRaw: parseTimeTo24Hour(apiRecord.check_in_time || apiRecord.checkInTime),
    checkOutRaw: parseTimeTo24Hour(apiRecord.check_out_time || apiRecord.checkOutTime),
    checkIn: formatTimeTo12Hour(apiRecord.check_in_time || apiRecord.checkInTime),
    checkOut: formatTimeTo12Hour(apiRecord.check_out_time || apiRecord.checkOutTime),
      ...
      setAttendanceData(prev =>
        prev.map(r =>
          r.id === editRecord.id
            ? {
                ...r,
                checkInRaw: editForm.checkIn ? editForm.checkIn : null,
                checkOutRaw: editForm.checkOut ? editForm.checkOut : null,
                checkIn: formatTimeTo12Hour(editForm.checkIn || null),
                checkOut: formatTimeTo12Hour(editForm.checkOut || null),
                status: editForm.status,
