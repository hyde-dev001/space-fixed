...
export default function RepairSupport() {
  ...
  const getReplySenderLabel = (message: NonNullable<Message['parentMessage']>) => {
    return message.sender === 'shop_owner'
      ? 'You'
  ...
  const getReplyContextLabel = (message: Message) => {
    ...
    if (message.sender === 'shop_owner') {
      return message.parentMessage.sender === 'shop_owner'
        ? 'You replied to yourself'
        : `You replied to ${selectedConversation?.customerName || selectedConversation?.shopName || 'Shop'}`;
    }

    return message.parentMessage.sender === 'shop_owner'
      ? `${selectedConversation?.customerName || 'Customer'} replied to you`
      : `${selectedConversation?.customerName || 'Customer'} replied to themselves`;
  };
      ...
      const messages = (Array.isArray(conv.messages) ? conv.messages : []).map((msg: any) => ({
        ...
        parentMessage: msg.parent_message
          ? {
              ...
              sender: msg.parent_message.sender_type === 'customer' ? 'customer' : 'shop_owner',
        ...
        const newMsg: Message = {
          ...
          parentMessage: messageData.parent_message
            ? {
                ...
                sender: messageData.parent_message.sender_type === 'customer' ? 'customer' : 'shop_owner',
                  ...
                  <div className="mb-3 bg-gray-50 border border-gray-200 rounded-2xl p-3 flex items-start justify-between shadow-sm">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-gray-500 font-semibold mb-1">
                        Replying to {replyingToMessage.sender === 'shop_owner' ? 'You' : (selectedConversation?.customerName || 'Customer')}
                      </p>
