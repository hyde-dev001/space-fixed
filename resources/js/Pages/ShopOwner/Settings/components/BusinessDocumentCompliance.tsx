						...
						<article key={slot.logical_slot} className="rounded-xl border border-gray-200 bg-gray-50/40 p-4">
							...
							{isActive && form && current && (
								<div className="mt-4 space-y-3 border-t border-gray-200 pt-4">
									...
									{slot.logical_slot === 'business_registration' && <label className="block text-xs font-semibold text-gray-700">Registration authority<MonochromeSelect value={form.documentType} onChange={(event) => setForm({ ...form, documentType: event.target.value })} className="mt-1 block w-full rounded-md border border-gray-300 px-2 py-1.5 text-sm"><option value="" disabled>Select issuing authority</option><option value="dti_registration">DTI</option><option value="sec_registration">SEC</option></MonochromeSelect></label>}
									...
									<label className="block text-xs font-semibold text-gray-700">Expiration<MonochromeSelect value={form.expirationMode} onChange={(event) => setForm({ ...form, expirationMode: event.target.value as 'dated' | 'none', expiresOn: event.target.value === 'none' ? '' : form.expiresOn })} className="mt-1 block w-full rounded-md border border-gray-300 px-2 py-1.5 text-sm"><option value="dated">Dated</option><option value="none">No expiration</option></MonochromeSelect></label>
