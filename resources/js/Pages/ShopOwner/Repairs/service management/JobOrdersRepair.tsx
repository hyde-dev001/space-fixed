...
export default function JobOrdersRepair() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const mappedOrders = response.data.data.map((repair: any) => {
          ...
          serviceType: currentOverrides[String(repair.id)] || ((repair.intake_delivery_method || repair.delivery_method) === 'walk_in' ? 'walkin' : 'pickup'),
  ...
  const handleActivatePickup = async (targetOrder: RepairOrder) => {
    ...
    try {
      const response = await axios.post(`/api/shop-owner/repairs/${targetOrder.database_id}/activate-pickup`);
  ...
  const handleReviewAction = async (action: "accept" | "reject" | "message") => {
    ...
    if (action === "accept" || action === "reject") {
      ...
      const confirmation = await Swal.fire({
        ...
        text: isAcceptAction
          ? isPosCreatedOrder
            ? "This will accept the walk-in repair request and continue without opening a customer chat."
        ...
        if (response.data.success) {
          ...
          await Swal.fire({
            ...
            text: shouldOpenChat
              ...
              : "The walk-in repair request was accepted.",
            icon: "success",
          ...
          if (shouldOpenChat) {
            window.location.href = `/shop-owner/repair-support?conversation_id=${createdConversationId}`;
          } else {
            setIsViewModalOpen(false);
            await fetchOrders();
          }
