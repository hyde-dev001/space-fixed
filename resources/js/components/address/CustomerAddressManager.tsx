      ...
      {modalOpen && (
        ...
        >
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-sm font-medium text-gray-800">Province<MonochromeSelect required value={form.province} onChange={(event) => setForm((current) => ({ ...current, province: event.target.value, region: event.target.value, city: '' }))} className="mt-1 min-h-11 w-full rounded-lg border border-gray-300 px-3"><option value="">Choose province</option>{PHILIPPINE_LOCATIONS.map(({ name }) => <option key={name}>{name}</option>)}</MonochromeSelect></label>
            <label className="text-sm font-medium text-gray-800">City/Municipality<MonochromeSelect required value={form.city} onChange={(event) => update('city', event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border border-gray-300 px-3"><option value="">Choose city or municipality</option>{cities.map((city) => <option key={city}>{city}</option>)}</MonochromeSelect></label>
