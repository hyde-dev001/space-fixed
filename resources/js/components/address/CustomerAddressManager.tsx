...
export default function CustomerAddressManager({ onSelect, initialAddressId = null, disabled = false }: Props) {
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="font-semibold text-gray-950">Delivery address</h2>
          <p className="text-sm text-gray-600">Choose where the rider should go. You can still use walk-in or your own courier.</p>
        </div>
      ...
      {editingId !== undefined && (
        <form onSubmit={save} className="space-y-4 rounded-xl border border-gray-200 bg-gray-50 p-4">
          ...
          <button type="submit" disabled={saving} className="min-h-11 w-full rounded-lg bg-gray-950 px-4 font-semibold text-white disabled:opacity-60">{saving ? 'Saving…' : editingId ? 'Save changes' : 'Save address'}</button>
        </form>
      )}
    </div>
  );
