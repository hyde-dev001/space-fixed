  ...
  return (
    <ul aria-label="Owner approval history" className="divide-y divide-gray-200 overflow-hidden rounded-xl border border-gray-200 dark:divide-gray-800 dark:border-gray-800">
      {items.map((item) => {
        ...
        return (
          <li key={item.attention_key} className={selected ? "bg-gray-950 text-white dark:bg-gray-950" : "bg-white dark:bg-white/[0.02]"}>
            <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
              <div className="min-w-0">
                ...
                <h3 className={`mt-1 truncate text-base font-semibold ${selected ? "text-white" : "text-gray-900 dark:text-white"}`}>{item.title}</h3>
                <p className={`mt-1 text-sm ${selected ? "text-gray-200" : "text-gray-600 dark:text-gray-300"}`}>{item.concise_summary}</p>
                <div className={`mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs ${selected ? "text-gray-300" : "text-gray-500 dark:text-gray-400"}`}>
              ...
              <button
                ...
                className={`inline-flex min-h-11 min-w-11 shrink-0 items-center justify-center self-start rounded-lg border transition focus:outline-none focus-visible:ring-2 focus-visible:ring-gray-950 focus-visible:ring-offset-2 ${selected ? "border-gray-600 text-white hover:bg-black dark:border-gray-600 dark:text-white dark:hover:bg-black" : "border-gray-300 text-gray-700 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"}`}
              >
