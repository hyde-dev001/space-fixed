...
- Seed **Urban Kicks Test Runner** with description, price `2499.00`, brand `Urban Kicks`, category `shoes`, active status, sizes 7–11, and Black/White colors.
- Upsert every size/color `ProductVariant` with quantity `100` and set parent stock to the matching total of `1000`, so each storefront selection is purchasable.
- Do not require an image, media upload, color-variant inventory hierarchy, or changes to `DatabaseSeeder`.
...
- The same test verifies exactly 10 unique size/color variants exist, each has quantity `100`, and reruns do not duplicate them.
