...
- The existing My Purchases lookup receives `has_failed_attempt`, the matching shipment ID, and the current leg's nullable `scheduled_delivery_date` (`Y-m-d`) and `delivery_window` (`morning`, `afternoon`, or null). It remains scoped to `source_type = order`, the exact order ID, and the latest shipment by `id DESC`; older shipments for the same order cannot drive the warning, estimate, or link.
...
- My Purchases renders this logistics estimate only when the existing `is_shop_owned_delivery` payload flag is true. A date with a window renders, for example, `July 18, 2026 · Morning`; a date without a window renders the date alone. A null date renders `Not scheduled yet` even if a window value exists. Third-party delivery keeps its existing carrier estimate behavior.
- The warning applies to every failed delivery reason, not only recipient unavailable.

## Testing

- Backend tests cover every exact reason-code mapping and the unknown fallback; latest-attempt ordering and tie-breaking; no-attempt and missing-proof cases; failed-then-delivered lifecycle; current-leg selection with other-leg failures kept historical; exact order matching; newest-shipment selection when an order has multiple shipments; estimate fields sourced only from the newest shipment's highest-sequence/current leg; full, partial, and null schedule serialization; and the absence of notes, recorder data, internal resolution details, and raw paths.
...
- Frontend tests cover the unresolved warning, reason/time/photo rendering, missing-photo fallback, successful-redelivery historical state, exact My Purchases order warning, correct tracking link, readable and partial shop-logistics estimates, the unscheduled fallback, and absence of the shop-logistics estimate for third-party delivery.
