...
- The existing My Purchases lookup receives `has_failed_attempt`, the matching shipment ID, and the current leg's nullable `scheduled_delivery_date` and `delivery_window`. It remains scoped to `source_type = order`, the exact order ID, and the latest shipment by `id DESC`; older shipments for the same order cannot drive the warning, estimate, or link.
...
- For shop-owned logistics, My Purchases shows the current leg's estimate as a readable date and delivery window (for example, `July 18, 2026 · Morning`). If no schedule exists, it shows `Not scheduled yet` instead of `-`.
...
- Frontend tests cover the unresolved warning, reason/time/photo rendering, missing-photo fallback, successful-redelivery historical state, exact My Purchases order warning, correct tracking link, readable shop-logistics estimate, and the unscheduled fallback.
