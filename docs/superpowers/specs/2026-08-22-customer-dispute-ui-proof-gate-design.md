...
The dispatcher shipment page will expose only the resolutions that have a supported workflow:

- `customer_confirmed` — closes the dispute after the customer confirms the delivery.
...
2. Resolve is hidden while a dispute is open and appears only after investigation starts.
3. A direct resolve request for an open dispute is rejected and leaves the dispute open.
4. Staff can click a delivery-proof thumbnail on Job Orders and view the full image in a modal, with an accessible close action.
5. Existing legacy resolution records remain readable and existing refund/return processing is not changed.
