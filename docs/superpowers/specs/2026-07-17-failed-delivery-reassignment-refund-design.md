...
3. Set the amount to the remaining refundable PayMongo capture after subtracting succeeded refunds. Treat every nonterminal refund, including `requested`, `pending_approval`, and `processing`, as a reservation. Every online refund creation path uses one shared `OrderRefundService` reservation transaction that locks the order/payment scope before calculating succeeded and reserved amounts and creating a request. If a different active refund exists, block creation of the failed-delivery refund and surface the collision for Finance resolution; retry creation after that request becomes terminal. Never let combined succeeded and reserved amounts exceed the captured amount. The failed-delivery amount includes shipping.
...
- `delivery_attempts.attempt_number` for display and limit accounting, plus `delivery_assignment_id` with a unique key on `(shipment_leg_id, attempt_type, delivery_assignment_id)`. The rider request carries the original assignment ID. Before transition validation, the service returns an existing attempt for that assignment if present; otherwise it locks and validates that exact active assignment, records the attempt, creates any custody-preserving return assignment, and closes it. Retrying after closure therefore recovers the same attempt instead of consuming another. Any non-assignment attempt endpoint must require a stable explicit idempotency key with a unique constraint;
